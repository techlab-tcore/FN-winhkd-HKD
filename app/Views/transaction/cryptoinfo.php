...
<section class="card bg-secondary my-3 shadow">
    <div class="card-body">
        ...
        <?=form_open('',['class'=>'form-validation cryptoForm','novalidate'=>'novalidate'],['amount'=>$_POST['amount'],'bank'=>$bankId,'card'=>$bankCardNo,'accno'=>$bankAccountNo,'promotion'=>$_POST['promotion'], 'currency'=>$_POST['currency']]);?>
        <div class="p-xl-5 p-lg-5 p-md-5 p-3 text-center">
            <img class="d-inline-block w-50" src="<?=$qrImg;?>">
        </div>
        ...
        <div class="row mb-3">
            <label class="col-xl-3 col-lg-3 col-md-3 col-4 col-form-label fs-xl-4 fs-lg-4 fs-md-4 fs-5"><?=lang('Input.accno');?>:</label>
            <div class="col-xl-9 col-lg-9 col-md-9 col-8">
                <input class="form-control form-control-lg" type="text" value="<?=$bankAccountNo;?>" readOnly>
            </div>
        </div>
        <div class="row mb-3">
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    $('.cryptoForm').off().on('submit', function(e) {
        e.preventDefault();

        if (this.checkValidity() !== false) {
            generalLoading();
            $('.cryptoForm [type=submit]').prop('disabled', true);
            
            const imgSource = $('.cryptoForm [name=receipt]')[0].files[0];
            const img = $('.cryptoForm [name=receipt]')[0].files[0]['name'];
            ...
            var params = {};
            var formObj = $('.cryptoForm').closest("form");
            ...
            if( params['promotion']!='' )
            {
                ...
                swal.fire({
                    ...
                    if( result.isConfirmed ) {
                        ...
                        // swal.close();
                        $('.cryptoForm [type=submit]').prop('disabled', false);
                    }
                });
            }
        }
...
function submitBankTransfer(params)
{
